...
| G02 | **Structural** | FS — BC 03 | **Cash Flow Statement not implemented.** BC 03 is legally required for annual FS per Circular 99 Article 17. Missing → cannot produce complete FS package → regulatory violation. | Implement FsService::generateBC03() with direct method (default) and indirect method option. Map cash account movements to operating/investing/financing categories. Validate closing cash (70) = BC 01 cash (111). | **HIGH** |
