...
# Compress agent context (headroom-ai) — giảm token cho LLM agent
# Yêu cầu: HEADROOM_BASE_URL + HEADROOM_API_KEY trong .env
cd accounting-app && npm run compress

# Dry-run: xem thống kê token
cd accounting-app && npm run compress:dry
...
|---|---|---|---|---|
| 4.9 | 2026-06-09 | **E-invoice XML import — 5 phases complete (migrations 132→137).** Phase 1: TCT XML parser, auto-create supplier/items/AP invoice + Nợ 156/1331/Có 331 journals, FKey dedup, 58 tests. Phase 2: Auto goods receipt + VAT summary endpoint, 22 tests. Phase 3: Bank payment tracking (unpaid→partial→paid lifecycle, over-payment prevention), 20 tests. Phase 4: Advance payment tracking (prepay_amount accumulate, multiple prepays), 12 tests. Phase 5: Production order allocation (raw_material/overhead/other categories, link to PO), 10 tests. Sidebar menu 'Lịch sử import HĐĐT' (tax section, sort_order 169). Dedicated view `einvoice_import_history.php` with filter, detail modal, export CSV. MISA competitive parity attained (5-module feature set). **Tests:** 122 e-invoice import tests, 0 failures. Full suite 85 files, ~2,150 tests, 1 pre-existing failure (multi-tenant backfill deferred). |
| 4.7 | 2026-06-09 | **E-invoice production readiness analysis.** Regulatory research vs GDT, EY, PwC, Deloitte, KPMG, ThuVienPhapLuat, Savitax. Full audit of existing e-invoice codebase (InvoiceService, XmlInvoiceBuilder, DigitalSignatureService, VnptEInvoiceGateway, VatService, VatDeclarationEngine, VatRateService, XbrlGenerator). Maturity score 7.8/10. 6 gaps identified (G01-G06). **Verdict:** PROD-ready with VNPT T-VAN, manual GDT submission acceptable interim. G01 (auto GDT submission) required before full go-live. 12 use cases (UC-01→UC-08), 20 business rules, 12 internal controls, 11 risks. Document at `docs/analysis/e-invoice-production-readiness.md` (743 lines). **Fixes:** journal.php:194/198, corrections.php:135 — `VAS.fmt(0)` HTML injection in `.text()` callers. All tests pass. **Migration 125:** request_id/request_number in petty_cash_transactions. PettyCashService.disburseFromRequest() — validates approved request → creates disbursement + auto markAsPaid in single txn. Controller + route POST /api/petty-cash/disburse-from-request. 03-TT view: 'Lập phiếu chi' button on approved, modal with fund selection, payment info on paid status. 21 tests (0 failures). **headroom-ai:** compress-context.mjs script, npm scripts (compress/compress:dry), .env.example, .gitignore update. |
| 4.5 | 2026-06-08 | **Mẫu 02-VT (Phiếu xuất kho) TT99 re-implementation — full-stack, end-to-end.** Form audit V1: all 116 views inventoried, 11 key forms assessed (15 categories each), report at `docs/analysis/form-audit-report.md`. Overall maturity 7.2/10. Mẫu 02-VT gap analysis vs TT99 — current `issue.php` is single-item modal, non-compliant. **Migration 122:** `inventory_issues` header + `inventory_issue_items` line tables. **Migration 123:** entity_id columns for multi-tenant compat. **New:** `GoodsIssue` model, `GoodsIssueService` (draft/post/cancel/list/get with AuditLoggerInterface DI), `IssueController` extended with `createDraft`/`postDraft`/`cancelDraft`/`getDetail`/`list` batch endpoints. **View:** Full-page Mẫu 02-VT redesign — multi-line grid, warehouse/receiver/reason fields, qty requested vs actual, unit price + amount per line, total in words, 5 signature slots, Draft→Posted→Cancelled lifecycle, print button. **Fixed:** `AuditLogger::log()`→`$this->auditLogger->log()` (static→DI), nested transaction in postIssue removed (issueGoods owns its own txn), LIMIT placeholder bound as int, undefined `$data['warehouse_id']` key guard. **Backward compat:** old single-item `/api/inventory/issue` unchanged. **Tests:** `GoodsIssueTest` — 27 tests (createDraft, postIssue, listIssues, cancelIssue, 4 failure cases). Full suite: ~1,964 tests / 78 files / 0 new failures (2 pre-existing: menu_items, menu_favorites). **Form audit V2:** 16 transaction forms assessed with 15-category 1-5 scoring framework via source code analysis at `docs/analysis/form-audit-report-v2.md`. Average score 3.08/5. Top: Period Close (4.3), Bottom: Goods Receipt (2.4). 20 critical issues identified, 15 quick wins, Standard Form Framework proposed, UX Guidelines + Enterprise Form Design Standards defined. |
| 4.4 | 2026-06-08 | **BC02 TT99 full BA+CA analysis v2.0 — expanded 14 sections.** Follows BC03 pattern: 6 use cases (UC-01 to UC-06 with happy + alternative paths), detailed data flow, business rules matrix (15 validation + 19 business rules), 16 gaps (P0-P3), 8-step workflow, user journeys, 13 internal controls, 4-phase implementation plan with file mapping. **Findings:** BC02 correct 18/21 indicators. P0/P1: XBRL map fix (5 wrong + 7 missing), MS 21 formula_detail, MS 24 sub-account 6351, gross loss warning. No material compliance risk. Web-verified vs einvoice.vn, safebooks.vn, thuvienphapluat. **Implementation:** Added `getBC02Warnings()` in FsService (BR18 gross loss + BR19 loss warning), wired to FsController::bc02() response and view (yellow alert), 10 new tests (88 total, 0 failed). **Gaps audit:** 13/16 gaps already closed by prior commits (v3.1 XBRL, migrations 116-118). Only G15 (gross loss warning) was truly open — now closed. **Test suite:** ~1,964 tests / 78 files / 0 new failures (1 pre-existing multitenant — deferred). **Decision:** Defer all multitenant features at this version. |
| 4.3 | 2026-06-08 | **BC03 TT99 full BA analysis + P0 implementation (3 gaps closed).** `docs/analysis/bc03-tt99-full-analysis.md` — 12 sections: structure, TT99 vs TT200 changes (3 diffs), seed data audit (10 items rated), XBRL mapping analysis, data flow, use cases (6 UC), rules matrix (12 validation + 11 business rules), gap analysis (11 gaps P1-P3), workflow, user journey, internal controls (10 IC), implementation recommendations (P0/P1/P2 phases). **G01 — XBRL BC03_MAP:** expanded from 7→37 items (all 4 sections). **G02 — Manual input UI:** `FsService::generateBC03($period, $manualValues)` accepts manual values, `FsController::saveManualValues()` now supports BC03 (statement field), new route `POST /api/fs/bc03/manual-values`, `fs_bc03.php` — 14 manual items editable inline with save button, `is_manual` flag in API response, `tt99()` passes manual values. **G03 — Prior period fallback** (verified already handled). **Fix:** Pre-existing `$val` undefined in `generateBC03Direct()`. **Tests:** 56 BC03 tests (was 16, +6 for manual values + is_manual + period scoping). Full suite: ~1,979 tests / 78 files / 0 new failures (1 pre-existing). |
| 4.1 | 2026-06-08 | **Dashboard redesign + BC01 imbalance fix + TT99 full BA analysis.** Dashboard: `DashboardService` aggregates 10+ KPIs (cash, revenue/expense YTD, pending approvals, trial balance, cash flow trend), `DashboardController` with `report.read` permission, 76 tests. **Bug fixes:** MenuService table names (`ap_transactions`→`ap_invoices`, `ar_transactions`→`ar_invoices`), SalesOrderController view path depth (4→5 `../`), 2 missing views (supplier_return, payroll_entry_detail). **BC01 fix (migration 114):** Line 163 "Thuế và khoản khác phải thu NN" wrongly included 333x (tax payable) accounts on asset side — 3334 (Thuế TNDN, 2.8M) double-counted on both asset and liability. Fixed: 163 = `1383` only. BC01 now balanced: 7M=7M. **TT99 full analysis:** `docs/analysis/bc01-tt99-full-analysis.md` — use cases, data flow, rules matrix (20 rules), gap analysis (14 gaps P0-P2), user journey, workflow, internal controls (10 controls). Cross-referenced vs ketoanthienung, MISA, EasyBooks, Fast, thuvienphapluat, BTC official. **BC09 seed fix (migration 115):** 39 indicators seeded with correct account codes (mapped from TT200→system). **Test suite:** 1,851 tests / 77 files / 0 new failures (1 pre-existing). |
...
| 3.1 | 2026-06-04 | **XBRL reporting (GDT submission format).** New service `XbrlGenerator` (`src/Accounting/Domain/Service/XbrlGenerator.php`) — XML output theo chuẩn XBRL 2.1 + namespace GDT `http://www.gdt.gov.vn/2025/btc`, hỗ trợ BC01 (instant), BC02/BC03 (duration), period formats: năm `2025` / tháng `2025-06` / quý `2025-Q2`. Map ma_so → GDT tags: TONGTAISAN, TONGNO, VONCSH, DOANHTHU, LOINHUAN, TIENCUOIKY, LCTT_HDKD... Skip unknown ma_so (forward-compatible). DOMDocument auto-escape; validate well-formed XML + namespace GDT. **Endpoints mới:** `GET /api/fs/xbrl/bc01|bc02|bc03?period=2025` trả `application/xml` + Content-Disposition attachment. **Wiring:** `config/services/33_financial.php` (XbrlGenerator factory), `config/services/40_controllers.php` (FsController giờ inject cả FsService + XbrlGenerator), `config/routes/api_financial.php` (3 routes mới). **Views:** Nút "Xuất XBRL (GDT)" trong `fs_bc01.php`, `fs_bc02.php`, `fs_bc03.php`. **RBAC:** `report.export` permission. **Validation:** Từ chối xuất nếu BC mất cân đối (HTTP 422). **Test:** 32 mới (`XbrlGeneratorTest`), 68 test files total, 0 failures. **Quyết định:** Placeholder URL `http://www.gdt.gov.vn/2025/btc/bc0X.xsd` — cập nhật khi GDT công bố chính thức. Tuân thủ TT 99/2025/TT-BTC cho DN >10B doanh thu. |
| 3.2 | 2026-06-04 | **P0 cluster spec — List Operations & Bulk Actions BA review complete.** Spec tổng hợp 6 nhóm chức năng + 10 BA gaps → 20 tasks (R-1 → R-20). **ADR-012: Multi-Tenant Single DB** (`docs/decisions/adr-012-multi-tenant-single-db.md`) — chọn Pattern B (column `company_id`) thay vì clone blank DB. Migrations 099-102: balance_lock, opening_balance_imports, companies + user_company_access, backfill `company_id` cho 40+ tables. 8 config keys mới: approval thresholds (cash 500M/2B, FA 1B, contract 5B), self-approve, rollback window 24h, opening balance lock, consolidation FX mode. **Quyết định chính:** Reverse Entry (bút toán ngược) cho Void; Bulk Post transactional all-or-nothing; Per-user saved filters; Import scope V1 = master data + opening balance (KHÔNG voucher); SME self-approve allowed. **Test growth target:** 68 → 97 test files (+29 mới cho multi-tenant). **Next:** Code R-1 (Reverse Entry) — foundation cho R-2, R-7, R-8, R-13. |
| 3.3 | 2026-06-04 | **R-1 Reverse Entry — implementation complete (commit c3bba95).** Phát hiện: `JournalService::createNegativeEntry()` + `CorrectionController::negative()` + route đã có sẵn (v2.5+). Gap thực sự: `Transaction::reverse()` không persist `reversedBy` → audit trail thiếu. **Fix:** (1) Migration 100 thêm `reversed_by` + `reversed_at` + composite index, (2) `Transaction::reverse()` lưu cả 2 field, (3) `PDOTransactionRepository::save()` + `findById()` persist/đọc 2 field, (4) thêm setter `setReversedBy`/`setReversedAt`. **Tests 11 mới** (JournalServiceTest 18→29): happy path + 3 failure cases (reverse non-posted, reverse non-existent, balance restore). Full suite 68 files / 0 failures. **Note:** ADR-012 cần update — codebase đã có `accounting_entities` + `transactions.entity_id` (migration 055) + `accounts.entity_id` (migration 078) + seed entity "HO" (Trụ sở chính"). Multi-tenant foundation thực tế 50% done, không phải green-field. **Next:** R-2 (View/Edit/Delete guard theo status) — needs R-1 done (✅). |
| 3.4 | 2026-06-04 | **R-2/R-3/R-8/R-9/R-13 — 5 tasks trong 1 commit (commit 6aa412e).** Vertical slice qua shared code touch points. **R-2 Status guard UI:** 'Đảo ngược' button + 'Đã đảo' badge trong journal view, JS function `reverseEntry()` gọi `/api/corrections/negative`. Edit/Delete append-only (không có routes → safe by default). **R-3 RBAC scope:** Auth helpers (`getCurrentUserId/Role`, `canViewAllData`), JournalController filter theo `created_by` nếu !canViewAllData. Config `rbac.scope_by_creator` (default false) + `rbac.entity_filter_enabled` (default true). **R-8 Bulk Post:** `JournalService::bulkPost(maxBatch=50)` all-or-nothing, pre-validate skip non-draft/pending. Bypass workflow state machine (go straight posted). **R-9 Duplicate:** `JournalService::duplicateEntry(?newDate)` copy lines → draft, description prefix `[COPY]`, audit link. **R-13 Soft Delete + Restore:** Migration 102 +columns, `isDeleted()` check, `softDelete(reason)` BLOCK posted (must reverse first), `restore(windowDays=30)` block outside window. PDOTransactionRepository:275 bug fix (hardcoded params → `$params`). **Tests 24 mới** (JournalServiceExtendedTest): R-9:7, R-13:6, R-8:6, R-3:4, R-13 window:1. Full suite 68→69 files / 1,428→1,452 tests / 0 failures. |
| 3.5 | 2026-06-04 | **R-4/R-5/R-6 — Import cluster (commit 3f76980).** **Migration 103** `import_batches` table (id, entity_type, file_hash SHA256, status pending/committed/rolled_back/failed, audit fields, error_log JSON). **ImportService** (5 entity types: items/customers/suppliers/coa/opening_balance): `validateCsv` (BOM strip, header normalize, row-level validation per schema), `commitBatch` (transactional, ON DUPLICATE KEY UPDATE, has_id/id_prefix/audit_columns per schema), `rollbackBatch` (24h window default + configurable via `import.rollback_window_hours`, opening_balance deletes rows + unlocks period via status=open, master data marks rolled_back only). **Pre-check:** `checkOpeningPeriodLock` — period must be open + 0 transactions. **Post-check:** `checkOpeningBalanceSum` — Dr=Cr ±0.01 + auto-lock period on success. **ImportController** 7 endpoints: supported-types, template (CSV BOM), dry-run, commit, rollback, batches list, batch detail. **Routes** 7 mới trong `api_financial.php`. **DI** wired trong `33_financial.php` + `40_controllers.php`. **Timezone fix:** `date_default_timezone_set('Asia/Ho_Chi_Minh')` ở `public/index.php` + `tests/bootstrap.php` — DB timestamp lưu theo VN timezone, PHP mặc định UTC → lệch 7h khi so sánh `strtotime` vs `time()`. **Tests 43 mới** (ImportServiceTest): happy + failure cases cho mỗi entity type. Full suite 69→70 files / 1,452→1,499 tests / 0 failures. **Quyết định:** Master data rollback v1 = mark-as-rolled_back only (data not deleted) — manual fix required. Opening balance rollback = delete rows + unlock period (full rollback). Items unique on `code`, opening_balance unique on `(account_code, period)`. |
| 4.0 | 2026-06-05 | **P0 cluster complete — all 20 R-tasks done in 8 commits.** **Inventory finding:** R-14, R-15 (routing rules + history) đã có sẵn từ v2.0 (approval_routing table + ApprovalRoutingService + ApprovalController). R-16, R-17 là phần còn thiếu. **R-10 Print Designer v1 (commit febf5cb):** Migrations 106+107 (print_templates + RBAC). PrintTemplateService với mini render engine (no lib deps) — `{{var}}` escape, `{{{var}}}` raw, `{{#if}}`/`{{#unless}}`/`{{#each}}` với `{{else}}` fallback, nested path, `@index`. 1MB size cap, 1000 iter cap, 16 depth cap. 3 default templates seeded (ap_invoice/ar_invoice/sales_order). Print button hook vào AP invoice view (resource_type=ap_invoice → join ap_invoices + suppliers). 37 tests mới. **R-16 Multi-level approval (commit 41dc315):** Migration 108 (approval_routing.approval_sequence JSON + journal_entry_approvals.approval_level). ApprovalRoutingService: `getRequiredApprovalSteps` (ordered array), `getCurrentApprovalLevel` (1-based count of existing approves), `isFullyApproved`. JournalService.approveEntry: track level, only set status=approved at final level. ApprovalController.userCanApprove: role check matches CURRENT level's role. Approve endpoint trả approval_level/required_levels/fully_approved. Seed: >100M = 2 cấp (KTT→GD), >1B = 3 cấp (KTT→GD→CFO), intercompany = 2 cấp (KTT→CFO). 20 tests mới. **R-17 Delegation (commit 58971df):** Migration 109 (approval_delegations: delegator→delegate, role, start/end date, reason, is_active, revoked_at). ApprovalRoutingService: `createDelegation` (self-delegate + end<=start rejected), `revokeDelegation`, `findActiveDelegationsFor` (NOW() BETWEEN filter), `listDelegations`, `getEffectiveRolesFor`. ApprovalController.approve: nếu user role không khớp current level → check delegation → allow + record `delegate_approve` action với on_behalf_of. 3 new endpoints: GET/POST /api/delegations, POST /api/delegations/:id/revoke. 18 tests mới. **R-18-20 Multi-tenant backfill (commit 2c93006):** Migration 110 — thêm entity_id INT UNSIGNED vào 98 tables, backfill về 1 (HO - Trụ sở chính), tạo idx_entity index. Skip 25 tables: _migrations, accounting_entities, users/user_roles/roles/role_permissions (RBAC global), business_config/period_config/bc09_config (config global), exchange_rates/voucher_sequences (global), HR tables, tax rates, uoms/warehouses, report_definitions. 38 tests mới. **Auth backward compat:** thêm 'read'/'update' synonyms cho 'view'/'edit' (R-11/R-12 đã dùng 'read' nhưng actionMap chỉ có 'view'). **Cumulative R-cluster:** 1,805 tests / 76 files / 0 failures. 8 commits (R-1, R-2/3/8/9/13, R-4/5/6, R-7, R-11, R-12, R-10, R-16, R-17, R-18-20). |
