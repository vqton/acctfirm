...
|---|---|---|---|---|
| 4.8 | 2026-06-09 | **Menu reorganization — migration 130.** Full menu audit: 4 misplaced items in `system` section identified as accounting domain (Hệ thống tài khoản → Nhập liệu, Số dư đầu kỳ → Nhập liệu, Giao dịch nội bộ → Nhập liệu, Quản lý kỳ → Xử lý cuối kỳ). Migration 130 moves them to `gl_report` with correct sub-heading parent_ids + sort_orders. **VAS.fmt(0) HTML injection fix:** `goods_receipt.php:353` — `fmt(data.total)` guard for zero values. **E-invoice production readiness analysis** (743 lines, maturity 7.8/10). **Tests:** 84 files, 81 pass / 1 pre-existing (multi-tenant backfill deferred). MenuServiceTest +8 assertions (moved items). |
