...
class AuthController
{
    ...
    public function login(): void
    {
        ...
        if (!$data || !isset($data['username'], $data['password'])) {
            Helpers::jsonError('username, password required');
            return;
        }
        ...
        if (!$user || !password_verify($data['password'], $user['password_hash'])) {
            Helpers::jsonError('Sai tên đăng nhập hoặc mật khẩu', 401);
            return;
        }
    ...
    {
        session_destroy();
        Helpers::jsonOk(['message' => 'Logged out']);
    }

    public function me(): void
    {
        if (!isset($_SESSION['user'])) {
            Helpers::jsonError('Not authenticated', 401);
            return;
        }
        Helpers::jsonOk([
            'user' => $_SESSION['user'],
