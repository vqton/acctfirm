...
class UserController
{
    ...
    public function list(): void
    {
        ...
        Helpers::jsonOk($stmt->fetchAll(\PDO::FETCH_ASSOC));
    }
    ...
    public function create(): void
    {
        ...
        if (!$data || !isset($data['username'], $data['password'], $data['full_name'])) {
            Helpers::jsonError('username, password, full_name required'); return;
        }
        $id = uniqid('u_');
    ...
    public function update(string $id): void
    {
        ...
        $user->execute([$id]);
        if (!$user->fetch()) { Helpers::jsonError('Not found', 404); return; }
    ...
    public function delete(string $id): void
    {
        ...
        if ($id === 'admin') { Helpers::jsonError('Cannot delete admin'); return; }
        $this->pdo->prepare('UPDATE users SET status = ? WHERE id = ?')->execute(['inactive', $id]);
        Helpers::jsonOk(['message' => 'Deactivated']);
    }
}
