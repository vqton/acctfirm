<?php
// Quản lý dữ liệu: danh mục vật tư hàng hóa
...
class PDOItemRepository implements ItemRepositoryInterface
{
    ...
    public function save(Item $item): void
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO items (id, code, name, item_type, unit, purchase_price, sale_price, stock_qty, min_stock, valuation_method_id, allow_negative_stock, description, status, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
             ...
             stock_qty=VALUES(stock_qty), min_stock=VALUES(min_stock), valuation_method_id=VALUES(valuation_method_id), allow_negative_stock=VALUES(allow_negative_stock), description=VALUES(description), status=VALUES(status)'
        );
        $stmt->execute([
            ...
            $item->getAllowNegativeStock() ? 1 : 0,
            $item->getDescription(),
    ...
    private function hydrate(array $row): Item
    {
        $item = new Item(
            ...
            $row['valuation_method_id'], (bool)($row['allow_negative_stock'] ?? false)
        );
