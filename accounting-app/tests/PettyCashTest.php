...
$pettyCash = new PettyCashService($accountRepo, $txnRepo, $journal, $pdo);
...
$fund = $pettyCash->establishPettyCash('Office petty cash', 5000000, 'tester');
...
$tx = $pettyCash->disbursePettyCash($fundId, 500000, 'Office supplies', 'PC-PC-01', 'tester');
...
$pettyCash->disbursePettyCash($fundId, 200000, 'Taxi fare', 'PC-PC-02', 'tester');
$pettyCash->disbursePettyCash($fundId, 800000, 'Tea & coffee', 'PC-PC-03', 'tester');

$txs = $pettyCash->getPettyCashTransactions($fundId);
assertTrue(count($txs) >= 3, '3 disbursements recorded');

$funds2 = $pettyCash->getPettyCashFunds();
...
$cash->recordReceipt(5000000, '511', 'Cash injection', 'PT-PC-01', 'tester');
$result = $pettyCash->replenishPettyCash($fundId, 642, 1500000, 'Monthly replenishment', 'PC-PC-04', 'tester');
assertTrue(isset($result['transaction_id']), 'Replenishment GL entry created');

$funds3 = $pettyCash->getPettyCashFunds();
...
$result2 = $pettyCash->closePettyCash($fundId, 5000000, 'tester');
assertTrue(isset($result2['transaction_id']), 'Close GL entry created');

$funds4 = $pettyCash->getPettyCashFunds();
