...
<div class="toolbar">
    ...
    <div><button class="btn btn-outline-success btn-sm me-1" onclick="exportCSV('#dataBody', 'danh-sach-kho')"><i class="bi bi-file-earmark-excel"></i> Xuất Excel</button><button class="btn btn-primary btn-sm" onclick="openCreate()"><i class="bi bi-plus-lg"></i> Thêm mới</button></div>
</div>
<div class="card-table">
    ...
    <table class="table">
        <thead><tr><th>Mã</th><th>Tên kho</th><th>Địa chỉ</th><th>Trạng thái</th><th class="text-center" style="width:90px"></th></tr></thead>
...
<form id="dataForm">
    ...
    <div class="modal-body">
        ...
        <div class="mb-2"><label>Tên kho *</label><input type="text" name="name" id="f_name" class="form-control form-control-sm" required></div>
        <div class="mb-2"><label>Địa chỉ</label><textarea name="address" id="f_address" class="form-control form-control-sm" rows="2"></textarea></div>
    </div>
...
function loadData() {
    fetch(API).then(function(r){return r.json();}).then(function(data){
        ...
        var rows=f.map(function(i){
            return '<tr><td>'+esc(i.code)+'</td><td>'+esc(i.name)+'</td><td>'+esc(i.address||'')+'</td><td>'+statusBadge(i.status?'active':'inactive')+'</td>'+
...
function openCreate(){editId=null;document.getElementById('dataForm').reset();document.getElementById('modalTitle').textContent='Thêm mới kho';$('#dataModal').modal('show');}
function openEdit(id){editId=id;fetch(API+'/'+id).then(function(r){return r.json();}).then(function(i){
    ...
    document.getElementById('f_address').value=i.address||'';document.getElementById('modalTitle').textContent='Sửa kho';$('#dataModal').modal('show');
