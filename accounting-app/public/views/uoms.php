...
<div class="toolbar">
    ...
    <div><button class="btn btn-outline-success btn-sm me-1" onclick="exportCSV('#dataBody', 'danh-sach-don-vi-tinh')"><i class="bi bi-file-earmark-excel"></i> Xuất Excel</button><button class="btn btn-primary btn-sm" onclick="openCreate()"><i class="bi bi-plus-lg"></i> Thêm mới</button></div>
</div>
<div class="card-table">
    ...
    <table class="table">
        <thead><tr><th>Mã</th><th>Tên đơn vị</th><th>Trạng thái</th><th class="text-center" style="width:90px"></th></tr></thead>
...
<form id="dataForm">
    ...
    <div class="modal-body">
        ...
        <div class="mb-2"><label>Tên đơn vị *</label><input type="text" name="name" id="f_name" class="form-control form-control-sm" required></div>
        <div class="mb-2"><label>Ghi chú</label><textarea name="description" id="f_description" class="form-control form-control-sm" rows="2"></textarea></div>
    </div>
...
function loadData() {
    fetch(API).then(function(r){return r.json();}).then(function(data){
        ...
        var rows=f.map(function(i){
            return '<tr><td>'+esc(i.code)+'</td><td>'+esc(i.name)+'</td><td>'+statusBadge(i.status?'active':'inactive')+'</td>'+
        ...
        document.getElementById('dataBody').innerHTML=rows||'<tr><td colspan="4" class="empty-state"><i class="bi bi-inbox"></i>Không có dữ liệu</td></tr>';
    });
}
function openCreate(){editId=null;document.getElementById('dataForm').reset();document.getElementById('modalTitle').textContent='Thêm mới đơn vị tính';$('#dataModal').modal('show');}
function openEdit(id){editId=id;fetch(API+'/'+id).then(function(r){return r.json();}).then(function(i){
    ...
    document.getElementById('modalTitle').textContent='Sửa đơn vị tính';$('#dataModal').modal('show');
