...
<div class="card-table">
  ...
  <table class="table"><thead><tr><th>Kỳ</th><th>NV</th><th>Tổng gross</th><th>BH DN</th><th>Thuế</th><th>Net</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
...
<div class="modal fade" id="periodModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
  ...
  <div class="modal-body">
    <div class="mb-3"><label class="form-label">Kỳ (YYYYMM)</label><input class="form-control" name="period_code" value="<?= date('Ym') ?>" required></div>
  </div>
...
function badge(s){if(s==='posted')return'<span class="badge-status badge-active">Đã ghi sổ</span>';if(s==='approved')return'<span class="badge-status badge-warning">Đã duyệt</span>';return'<span class="badge-status" style="background:#e5e7eb;color:#374151;">Nháp</span>';}
...
$('#periodForm').submit(function(e){e.preventDefault();$.post('/api/payroll/periods',JSON.stringify({period_code:$('#periodForm [name=period_code]').val()})).done(function(){showToast('Đã tạo kỳ lương','success');$('#periodModal').modal('hide');loadPeriods();}).fail(function(x){showToast(x.responseJSON?.error||'Lỗi','error');});});
...
$('#processForm').submit(function(e){e.preventDefault();$.post('/api/payroll/process',JSON.stringify({period_id:$('#processPeriodSelect').val()})).done(function(r){showToast('Đã tính lương cho '+r.data?.total_employees+' nhân viên','success');$('#processModal').modal('hide');loadEntries();}).fail(function(x){showToast(x.responseJSON?.error||'Lỗi tính lương','error');});});
function closePeriod(id){if(!confirm('Đóng kỳ lương này?'))return;$.post('/api/payroll/periods/'+id+'/close').done(function(){showToast('Đã đóng kỳ lương','success');loadPeriods();}).fail(function(x){showToast(x.responseJSON?.error||'Lỗi','error');});}
function approveEntry(id){if(!confirm('Duyệt bảng lương này?'))return;$.post('/api/payroll/entries/'+id+'/approve').done(function(){showToast('Đã duyệt','success');loadEntries();}).fail(function(x){showToast(x.responseJSON?.error||'Lỗi','error');});}
function postEntry(id){if(!confirm('Ghi sổ bảng lương? Thao tác này không thể hoàn tác.'))return;$.post('/api/payroll/entries/'+id+'/post').done(function(){showToast('Đã ghi sổ','success');loadEntries();}).fail(function(x){showToast(x.responseJSON?.error||'Lỗi','error');});}
function showDetails(id){$.get('/api/payroll/entries/'+id+'/details').done(function(r){var h='<table class="table table-sm"><thead><tr><th>NV</th><th>Gross</th><th>BH</th><th>Thuế</th><th>Net</th><th>CP DN</th></tr></thead><tbody>';if(r.data?.details){r.data.details.forEach(function(d){h+='<tr><td>'+esc(d.employee_code)+' - '+esc(d.employee_name)+'</td><td>'+fmt(d.gross_salary)+'</td><td>'+fmt(d.insurance_ee)+'</td><td>'+fmt(d.tax_amount)+'</td><td>'+fmt(d.net_pay)+'</td><td>'+fmt(d.total_cost)+'</td></tr>';});}h+='</tbody></table>';showToast('','info');var m=$('<div class="modal fade" tabindex="-1"><div class="modal-dialog modal-xl"><div class="modal-content"><div class="modal-header"><h5>Chi tiết bảng lương</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body">'+h+'</div></div></div></div>');m.modal('show');m.on('hidden.bs.modal',function(){m.remove();});});}
