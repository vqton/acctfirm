...
function loadData() {
    fetch(API).then(function(r){return r.json();}).then(function(data){
        ...
        var rows=f.map(function(i){
            ...
            return '<tr><td>'+esc(i.code)+'</td><td>'+esc(i.name)+'</td><td>'+esc(i.method||'')+'</td><td class="text-end">'+(i.default_life||'')+'</td><td class="text-end">'+(i.default_salvage_rate||0)+'%</td><td><span class="badge badge-status '+sc+'">'+st+'</span></td>'+
