...
function loadAccounts(){
    ...
    var timedOut=false;var timer=setTimeout(function(){console.log('[Cash] TIMEOUT fired');timedOut=true;$('#loadStatus').text('TIMEOUT - Kiểm tra server').css('color','red');$('#debitAccount').html('<option>Không phản hồi (>5s)</option>');},5000);
    $('#debitAccount').html('<option>Đang tải...</option>');
    $.get('/api/cash/accounts?for=payment&_='+Date.now(),function(l){
        ...
        if(timedOut)return;clearTimeout(timer);
        $('#debitAccount').html('<option>Lỗi tải: '+x.status+'</option>');
