...
<div class="toolbar">
    ...
    <div><button class="btn btn-outline-success btn-sm me-1" onclick="exportCSV('#dataBody', 'danh-sach-du-an')"><i class="bi bi-file-earmark-excel"></i> Xuất Excel</button><button class="btn btn-primary btn-sm" onclick="openCreate()"><i class="bi bi-plus-lg"></i> Thêm mới</button></div>
</div>
    ...
    fetch('/api/customers').then(function(r){return r.json();}).then(function(customers){
        ...
        fetch(API).then(function(r){return r.json();}).then(function(data){
            ...
            var rows=f.map(function(i){
                ...
                return '<tr><td>'+esc(i.code)+'</td><td>'+esc(i.name)+'</td><td>'+cust+'</td><td>'+(i.start_date||'')+'</td><td>'+(i.end_date||'')+'</td><td class="text-end">'+(i.budget||0).toLocaleString()+'</td><td>'+statusBadge(i.status?'active':'inactive')+'</td>'+
