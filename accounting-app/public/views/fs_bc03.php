...
function loadData() {
    ...
    $.get('/api/fs/bc03?period='+period, function(res) {
        ...
        res.items.forEach(function(r){
            ...
            tbody.append('<tr class="'+cls+'"><td>'+esc(r.ma_so)+'</td><td>'+esc(r.name_vi)+'</td><td class="text-end font-monospace">'+val+'</td><td class="text-end font-monospace text-muted">'+pVal+'</td></tr>');
        });
    });
}
