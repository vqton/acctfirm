...
function loadData() {
    fetch(API).then(function(r){return r.json();}).then(function(data){
        ...
        var rows=f.map(function(i){
            ...
            return '<tr><td>'+esc(i.code)+'</td><td>'+esc(i.name)+'</td><td>'+esc(i.unit||'')+'</td><td>'+(i.quantity||0)+'</td><td>'+esc(i.allocation_type||'')+'</td><td class="text-end">'+(i.total_cost||0).toLocaleString()+'</td><td class="text-end">'+(i.allocated||0).toLocaleString()+'</td><td><span class="badge badge-status '+sc+'">'+st+'</span></td>'+
