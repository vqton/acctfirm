<?php
// Màn hình in Phiếu thu (Mẫu 01-TT) theo TT 99/2025/TT-BTC
// NGHIỆP VỤ: In phiếu thu tiền mặt — mẫu bắt buộc theo Thông tư 99/2025/TT-BTC
//
// Mẫu số 01-TT: Phiếu thu
// Quy định:
//   - Bắt buộc có chữ ký: Người lập phiếu, Kế toán trưởng, Thủ quỹ, Người nộp tiền, Giám đốc
//   - Số tiền phải viết bằng số và bằng chữ
//   - Kèm theo chứng từ gốc: ghi rõ số lượng
//   - Có dòng "Đã nhận đủ số tiền" cho Thủ quỹ ký
//   - Ghi rõ tài khoản Nợ (111, 1111) để đối ứng
//
// RỦI RO: Nếu thiếu dòng "Đã nhận đủ số tiền", phiếu thu không có giá trị thanh toán
// Hậu quả: Kiểm toán từ chối, tranh chấp với người nộp

$title = 'Phiếu thu - Mẫu 01-TT';
$txnId = $_GET['id'] ?? '';
if (!$txnId) { echo 'Thiếu mã phiếu thu'; exit; }
...
if (!$txn) { echo 'Không tìm thấy phiếu thu'; exit; }
...
$documentCount = $txn['document_count'] ?? '';
ob_start(); ?>
<!DOCTYPE html>
<html lang="vi">
<head><meta charset="utf-8"><title>Phiếu thu - Mẫu 01-TT</title>
<style>
    ...
    .received-confirm { text-align: center; font-size: 14px; font-weight: bold; margin: 24px 0 8px; padding: 8px; border: 1px solid #333; }
...
<table class="detail">
    <tr><td class="label">Người nộp tiền:</td><td><?= esc($txn['payer_name'] ?? '') ?></td></tr>
    <tr><td class="label">Địa chỉ:</td><td><?= esc($txn['payer_address'] ?? '') ?></td></tr>
    <tr><td class="label">Lý do nộp:</td><td><?= esc($txn['description'] ?? '') ?></td></tr>
</table>
...
<table class="detail" style="border:1px solid #333;">
    ...
    <tr>
        ...
        <td><?= esc($l['note'] ?? ($l['is_debit'] ? 'Tiền mặt' : 'Thu')) ?></td>
...
<div class="received-confirm"><em>Đã nhận đủ số tiền: <?= esc($amountWords) ?></em></div>
